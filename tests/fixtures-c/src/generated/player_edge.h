// GENERATED BY cyclonec
// DO NOT EDIT MANUALLY
// source: src/models/player.h
// model: Player
// codec: edge
// fingerprint: sha256:b6fb9cf5777c645b7b14b4bcdb6b3e60d69109cbc704f6774901c9b63b6e71ac
// cyclonec-version: 0.2.0
// generated-at: 2026-08-20T06:05:20Z

#pragma once

#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>

#include "runtime.h"
#include "src/models/player.h"

// The "edge" codec for Player, generated from its Cyclone markers.
//
// The wire layout, in declaration order (RFC-0002 SS5.1):
//
//  0. `Id`: `u32`
//  1. `X`: `f32`
//  2. `Y`: `f32`
//  3. `Name`: `string`
//  4. `Payload`: `bytes`

static const char *const PlayerEdgeCodec_MESSAGE_NAME = "Player.edge";
static const uint32_t PlayerEdgeCodec_MESSAGE_ID = 0x432AB486u;
static const uint64_t PlayerEdgeCodec_FINGERPRINT = 0xB6FB9CF5777C645BULL;

// Writes the "edge" fields of `*value`, in declaration order. Returns `false` (buffer
// untouched beyond what was already written) if growing the writer's buffer failed.
static inline bool PlayerEdgeCodec_encode(CycloneWriter *writer, const struct Player *value) {
    if (!cyclone_writer_write_u32(writer, value->Id)) return false;
    if (!cyclone_writer_write_f32(writer, value->X)) return false;
    if (!cyclone_writer_write_f32(writer, value->Y)) return false;
    if (!cyclone_writer_write_string(writer, value->Name)) return false;
    if (!cyclone_writer_write_bytes(writer, &value->Payload)) return false;
    return true;
}

// Reads the "edge" fields into `*value`, in declaration order.
//
// Fields this codec does not carry are left as they were, which is what lets one
// model be split across several codecs.
//
// A field the stream ended before takes its zero value (RFC-0002 SS9.1); a field
// the stream ended inside is an error. Bytes left over after the last field
// belong to a newer writer's model and are ignored.
//
// `*value` must not already hold data from a previous, unfreed decode - see
// runtime.h's module docs.
static inline CycloneDecodeError PlayerEdgeCodec_decode(CycloneReader *reader, struct Player *value) {
    if (cyclone_reader_field_absent(reader)) {
        value->Id = 0;
    } else {
        CycloneDecodeError error = cyclone_reader_read_u32(reader, &value->Id);
        if (!cyclone_decode_error_ok(&error)) return error;
    }
    if (cyclone_reader_field_absent(reader)) {
        value->X = 0.0f;
    } else {
        CycloneDecodeError error = cyclone_reader_read_f32(reader, &value->X);
        if (!cyclone_decode_error_ok(&error)) return error;
    }
    if (cyclone_reader_field_absent(reader)) {
        value->Y = 0.0f;
    } else {
        CycloneDecodeError error = cyclone_reader_read_f32(reader, &value->Y);
        if (!cyclone_decode_error_ok(&error)) return error;
    }
    if (cyclone_reader_field_absent(reader)) {
        value->Name = NULL;
    } else {
        CycloneDecodeError error = cyclone_reader_read_string(reader, &value->Name);
        if (!cyclone_decode_error_ok(&error)) return error;
    }
    if (cyclone_reader_field_absent(reader)) {
        value->Payload.data = NULL;
        value->Payload.len = 0;
    } else {
        CycloneDecodeError error = cyclone_reader_read_bytes(reader, &value->Payload);
        if (!cyclone_decode_error_ok(&error)) return error;
    }
    return cyclone_decode_ok();
}
